...
- **自动提取关键词**：不传关键词直接输入记忆，系统自动提取（配置 LLM 时由大模型抽取，否则规则兜底：中文取高频片段/二元组，英文取词频）。
- **防止块膨胀**：新片段的关键词被某块完全覆盖时，只吸收进最强块，不做多余合并；只有真正桥接多个块的片段才触发合并。
- **遗忘机制**：块强度随时间衰减（默认 30 天半衰期），被检索/进入时加强（用进废退）；强度过低的弱块可 `prune` 归档清理（归档而非删除，可追溯）。

## 在线演示

打开 [Ti-memory 展示页](https://gptchaser.github.io/Ti-memory/) 可在线体验：交互 Demo 在浏览器内模拟记忆的吸收、合并与通路识别，行为与 Python 引擎一致。页面源码位于 `docs/`。
...
python cli.py add "每天早上骑车去公司"            # 不传关键词, 自动提取
python cli.py list                                # 列出记忆块
python cli.py stats                               # 记忆统计(块数 vs 片段数)
python cli.py query B                             # 关键词检索(概览 + 关联块)
python cli.py get <block_id>                      # 进入块内看整体描述
python cli.py prune --threshold 0.2               # 归档长期不用的弱块
```

`demo.py` 依次演示：片段吸收与淡出、融会贯通通路、关键词自动提取、块总数统计、遗忘与归档，并生成 `demo_output/before.svg` 与 `after.svg`。

## 可选：接入大模型生成整体描述

未配置时使用规则版渲染（确定性、可测试）。配置以下环境变量后，融会贯通时由 LLM 生成更自然连贯的整体描述，`add` 不传关键词时也由 LLM 抽取关键词：
...
│   ├── extract.py       #   关键词自动提取(LLM 可选 + 规则兜底)
